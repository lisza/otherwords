...
var populateGrid = function populateGrid(results) {
  Object.keys(results).forEach(function (key) {
    ...
    if (parentElement) {
      var _wordDiv = document.createElement('div');
      _wordDiv.id = key;
      _wordDiv.innerHTML = word.text;

      // Add tooltip span element inside of div
      var tooltip = document.createElement('span');
      tooltip.className = "tooltiptext";
      tooltip.innerHTML = word.language;
      _wordDiv.appendChild(tooltip);

      // Show language tooltips onclick for touch screens
      addOnclickEvent(tooltip);

      // Calculate border color based on word distance
      _wordDiv.style.border = '1px solid rgb(180, ' + word.distance * 40 + ', 180)';

      parentElement.appendChild(_wordDiv);
    }
  });
};

var addOnclickEvent = function addOnclickEvent(tooltip) {
  ...
  if ('ontouchstart' in document) {
    wordDiv.onclick = function () {
      ...
      var oldToltips = document.getElementsByClassName('visible');
